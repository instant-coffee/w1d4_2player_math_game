...
def question_round(player)
  ...
  if correct_answer
    ...
    puts "that wasn't the answer we were looking for, #{player[:name]}"
    1
  end
end
...
def display_scores(player1, player2)
  ...
  print " (WINNER WINNER CHICKEN DINNER)" if player_one_wins
  print "Player 2 (#{player2[:name]}): #{player2[:lives]}"
  print " (YOU'VE GOT THE POWER, YOU'VE GOT THE TOUCH)" if player_two_wins
end
...
until play_again == quit_now_tag
  ...
  play_again_tag = "y"
  print "play again? (#{play_again_tag}/#{quit_now_tag}) "
